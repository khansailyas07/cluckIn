...
<ul class="navbar-nav">
    ...
    @if (\App\SystemAddons::where('unique_identifier', 'pos')->first() != null && \App\SystemAddons::where('unique_identifier', 'pos')->first()->activated == 1)
        @if (Auth::user()->type != 1)
        ...
        <li class="nav-item mb-2 fs-7 dropdown multimenu {{ Auth::user()->type != 1 ? (in_array('25', $modules) == true ? '' : 'd-none') : '' }}" id="25">
            <a class="nav-link collapsed rounded d-flex align-items-center justify-content-between dropdown-toggle mb-1" href="#pos" data-bs-toggle="collapse" role="button" aria-expanded="false" aria-controls="pos">
                <span class="d-flex"><i class="fa-solid fa-bag-shopping"></i> <span class="multimenu-title">{{ trans('labels.pos') }} @if(env('Environment') == 'sendbox') <small class="badge bg-danger">Addon</small> @endif</span>
            </a>
    ...
    @if (\App\SystemAddons::where('unique_identifier', 'otp')->first() != null && \App\SystemAddons::where('unique_identifier', 'otp')->first()->activated OR env('Environment') == 'sendbox')
        <li class="nav-item mb-2 fs-7 {{ Auth::user()->type != 1 ? (in_array('26', $modules) == true ? '' : 'd-none') : '' }}" id="26">
            <a class="nav-link rounded d-flex {{ request()->is('admin/otp-configuration*') ? 'active' : '' }}" href="{{ URL::to('/admin/otp-configuration') }}" aria-expanded="false">
                <i class="fa-solid fa-key-skeleton"></i><span class="nav-text ">{{trans('labels.otp_configuration')}} @if(env('Environment') == 'sendbox') <small class="badge bg-danger">Addon</small> @endif</span>
            </a>
        </li>
    @endif
    @if (Auth::user()->type != 1)
    ...
    <li class="nav-item mb-2 fs-7">
        <a class="nav-link rounded d-flex {{ request()->is('admin/subscribe*') ? 'active' : '' }}" href="{{ URL::to('/admin/subscribe') }}" aria-expanded="false">
    ...
    <li class="nav-item mb-2 fs-7 dropdown multimenu {{ Auth::user()->type != 1 ? (in_array('21', $modules) == true ? '' : 'd-none') : '' }}" id="21">
        ...
        <ul class="collapse" id="pages">
            ...
            <li class="nav-item ps-4 mb-1">
                <a class="nav-link rounded {{ request()->is('admin/refundpolicy*') ? 'active' : '' }}" aria-current="page" href="{{ URL::to('/admin/refundpolicy') }}">
    ...
    <li class="nav-item mb-2 fs-7 {{ Auth::user()->type != 1 ? (in_array('28', $modules) == true ? '' : 'd-none') : '' }}" id="28">
        <a class="nav-link rounded d-flex {{ request()->is('admin/whatsapp_settings*') ? 'active' : '' }}" href="{{ URL::to('/admin/whatsapp_settings') }}" aria-expanded="false">
        <i class="fa-brands fa-whatsapp"></i><span class="nav-text ">{{ trans('labels.whatsapp_settings') }}</span>
        </a>
    </li>
    @endif
    <li class="nav-item mb-2 fs-7" {{ Auth::user()->type != 1 ? (in_array('27', $modules) == true ? '' : 'd-none') : '' }}" id="27">
        <a class="nav-link rounded d-flex {{ request()->is('admin/language-settings*') ? 'active' : '' }}" href="{{ URL::to('/admin/language-settings') }}" aria-expanded="false">
            <i class="fa-solid fa-language"></i><span class="nav-text ">{{ trans('labels.language') }}</span>
        </a>
    </li>
